...
2. **Pre-creates "no-fold" directories** (`~/.config/openlogi`). See [Layout: NO_FOLD_DIRS](#layout-no_fold_dirs) below.
3. **`stow --restow`** — symlinks everything into `$HOME`.

Re-run after pulling updates, editing the `Brewfile`. The script is idempotent — safe to run any time.
...
Add an entry to `NO_FOLD_DIRS` (in `install.sh`) whenever you track a tool that writes runtime files alongside its config.
